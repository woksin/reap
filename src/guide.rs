...
pub const GUIDE: &[Section] = &[
    ...
    Section {
        ...
        body: &[
            "Most candidates outside Personal have an owner that can remake them — a",
            "compiler, package manager, or browser. Nothing in Personal proves that.",
            "",
            "So reap does not guess. Names ending in .dmg, .exe, .pkg or .iso are",
            "grouped as installers for review, but remain irreversible: an extension",
            "cannot prove the file is public or that another copy exists. Re-grade one",
            "only when you know it can be downloaded again. Everything else is also",
    ...
    Section {
        ...
        body: &[
            ...
            "days. reap checks ancestry, patch identity, remotes and local state, and",
            "fails closed when those checks cannot prove the work survives elsewhere.",
            "",
    ...
    Section {
        ...
        body: &[
            ...
            "Commands such as git branch, docker prune and package-manager clean are",
            "not path moves, so --trash does not make those commands recoverable.",
        ],
    },
    Section {
